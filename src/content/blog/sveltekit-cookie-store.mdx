...
This script uses the `js-cookie` package to manage cookies. And for the theme switcher, it uses the `@sveltekit-addons/document` package to change the `<html>` tag's `data-theme` and `class` attributes.
...
### Adding the document preprocessor
...
export function serverGetCookie<T extends string | number | boolean | object>(cookies: Cookies, name: string, defaultCookie: T) {
	...
	const cookie = val ? (JSON.parse(val) as typeof defaultCookie) : defaultCookie;
