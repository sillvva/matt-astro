...
The `App.Cookie` type used in the load function [above](#usage) is based on the types in `constants.ts`. `app.settings.theme` is of type `Themes`, which is a union of the available theme names, and `app.settings.mode` is of type `ThemeGroups`, which is a union of `"dark" | "light"`.
