...
const renameWebAuthn = async (id?: string, currentName = "") => {
  ...
  const value = (await response.json()) as { success: true; name: string } | { success: false; error: string };
...
async function deleteWebAuthn(id: string) {
  ...
  if (confirm(`Are you sure you want to delete "${authName(auth)}"?`)) {
    ...
    const value = (await response.json()) as { success: true } | { success: false; error: string };
...
<ul class="menu menu-lg w-full px-0 [&_li>*]:px-2">
  ...
  {#each authenticators as authenticator}
    {@const name = authenticator.name || authenticator.credentialID.replace(/[^a-z0-9]/gi, "").slice(-8)}
    <li>
      <button class="flex gap-2" on:click={() => renameWebAuthn(authenticator.credentialID, name)}>
        <span class="iconify size-6 material-symbols--passkey"></span>
        <span class="w-44 flex-1 overflow-hidden text-ellipsis whitespace-nowrap">
          {name}
        </span>
        <button
  ...
  <li>
    <button
      type="button"
      on:click={() =>
        passkey("webauthn", { action: "register", redirect: false })
          .then((resp) => {
            if (resp?.ok) renameWebAuthn();
