...
import type { Authenticator } from "$server/db/schema";
...
export async function POST({ request, locals }) {
	...
	try {
		const auth = await db.query.authenticators.findFirst({
			where: (table, { eq, and }) => and(eq(table.userId, session.user.id), id ? eq(table.credentialID, id) : eq(table.name, ""))
		});

		if (!auth) throw new Error("No passkey found");
		if (!name.trim()) name = auth.name || auth.credentialID.replace(/[^a-z0-9]/gi, "").slice(-8);
...
$: authenticators = $page.data.authenticators as Authenticator[];
