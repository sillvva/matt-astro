---
title: DaisyUI Theme Switcher for SvelteKit
...
```html
<!doctype html>
<html lang="en" %mode% %theme%> <!-- [!code focus] [!code highlight] -->
	<head>
		<meta charset="utf-8" />
...
const preloadTheme: Handle = async ({ event, resolve }) => {
	// Load user's theme preferences from cookies or wherever you store them
	const { settings } = event.locals.app;
	const mode = settings.mode;
	
	// Use specific theme for app routes, fallback to mode for others
	const theme = event.route.id?.startsWith("/(app)") ? settings.theme : settings.mode;
