...
updated: 2023-09-26T22:31:28.933Z
...
You will need to have a validation library installed. This component works with almost every validation library, including zod, valibot, etc. For this example, I am using valibot.
...
<CombinedBlock>
	...
	<script lang="ts" context="module">
		...
		export async function parseFormData<TSchema extends Schema>(formData: FormData, schema: TSchema): Promise<Infer<TSchema>> {
			...
			if ("issues" in result && result.issues.length) {
				throw new Error(result.issues.map((i) => `❌ ${(i.path || ["form"]).join(".")}: ${i.message}`).join("\n"));
			}
	...
	<script lang="ts" generics="TSchema extends Schema">
		...
		export let errors = new SvelteMap<"form" | Paths<InferIn<TSchema>, 6>, string>();
		...
		function setError<K extends "form" | Paths<InferIn<TSchema>, 6>>(path: K, message: string) {
...
export const imageSchema = union([
	...
	blob([
		mimeType(["image/png", "image/jpeg", "image/gif", "image/webp"], "Must be an image"), 
		maxSize(5 * 1024 * 1024, "Must be less than 5MB")
	]),
