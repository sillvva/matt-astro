...
function getset<TGets extends object, TSets extends object>(gets: TGets, sets: TSets) {
	if (gets && typeof gets != "object") throw new Error("gets must be an object");
	if (sets && typeof sets != "object") throw new Error("sets must be an object");

	const keys = [...new Set([...Object.keys(gets), ...Object.keys(sets)])];
	if (!keys.length) throw new Error("gets must not be empty");
	
	let out = {} as Prettify<TGets & TSets>;
	for (let key of keys) {
		if (sets && typeof sets[key] != "undefined") {
			let state = $state(sets[key]);
