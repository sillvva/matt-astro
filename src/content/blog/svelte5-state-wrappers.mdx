...
This utility function is useful for creating reactive objects from JSON data. The idea was first created by this [Twitter user](https://twitter.com/dimfeld/status/1704568707052159097) and modified by [@PaoloRicciuti](https://twitter.com/PaoloRicciuti). I added TypeScript types and fixed an issue with arrays and effects. Some array issues still remain.
...
```ts
export function deepState<T extends object>(obj: T): T {
	if(typeof obj === 'object') {
		let rune = $state(obj);
		for(let key in rune) {
			rune[key] = deepState(rune[key] as T);
		}
		return new Proxy(rune, {
			...
			set(target, prop, value) {
				if (Array.isArray(value)) {
					target[prop] = deepState(value as T);
					return true;
				}
				if (typeof target[prop] === "object" && "$" in target[prop]) {
					target[prop].$ = value;
...
`@intrnl` on the [Svelte Discord](https://discord.gg/svelte), created an alternative based on the `createMutable()` function from [Solid](https://www.solidjs.com/docs/latest/api#createmutable). This version is more complex, but it resolves some of the issues with the above version.

<div class="button-row">
	[Playground](btn::/r/svelte5-reactive-repl)
	[Source](btn::https://gist.github.com/intrnl/9b9313184df1763c5a12cbff3d975c4a)
</div>

### State Injection

`@elstalky` on the Discord also came up with a builder to inject reactive properties into an existing static object.

<div class="button-row">
	[Playground](btn::https://www.typescriptlang.org/play?jsx=0#)
</div>

```ts
function build<T>(obj: T) {
	function writables<O extends Record<string, any>>(props: O) {
		for (const key in props) {
			let state = $state(props[key]);
		...
		return build(obj as T & O)
	}

	function finish() { return obj }
