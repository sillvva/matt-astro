...
function getset<TSets extends object, TGets extends object>(writeables: TSets, readonly: TGets) {
	if (writeables && typeof writeables != "object") throw new Error("writeables must be an object");
	if (readonly && typeof readonly != "object") throw new Error("readonly must be an object");

	const keys = [...new Set([...Object.keys(readonly), ...Object.keys(writeables)])];
	if (!keys.length) throw new Error("gets must not be empty");
	
	let out = {} as Prettify<TGets & TSets>;
	for (let key of keys) {
		if (writeables && typeof writeables[key] != "undefined") {
			let state = $state(writeables[key]);
...
const fb = foobar();
fb.foo = 1; // OK, has getter and setter
fb.bar = 1; // Error, has no setter
```