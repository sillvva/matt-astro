...
The `toForm` method of the `SaveError` class can be returned from the form action to display the error message to the user. 

The `handleSaveError` function can be used to handle other types of errors that occur during the save process and convert them to `SaveError` objects.
...
## Using the `SaveResult` Type and the `SaveError` Class
...
export async function saveLog(input: LogSchema, user?: Session["user"]): SaveResult<LogData, LogSchema> {
    ...
    ...
		...
		return handleSaveError(err); // or SaveError<LogSchema>
	}
}
```
