...
updated: 2024-04-04T14:07:42.769Z
...
## Accounting for Branded Types

### What Are Branded Types?

Branded types are types that are created by using a unique symbol as a property key. This allows you to create types that are compatible with other types but are distinct from them. For example, you can create a branded type for a string that represents a URL and use it in place of a regular string.

This [article](https://egghead.io/blog/using-branded-types-in-typescript) provides a detailed explanation of branded types in TypeScript. 
...
const car = createCar("Toyota", "Corolla", "Diesel", "Red")
// "Diesel" is not of type "EngineType"
```

### Making `SaveError` Compatible with Branded Types

The following code demonstrates how to make the `SaveError` class compatible with branded types. The `ExtractBrand` type is used to extract the brand-related keys from the form paths.
...
```ts
import { dev } from "$app/environment";
import { type ExtractBrand } from "$lib/brand"; // [!code focus] // [!code ++]
import { type NumericRange } from "@sveltejs/kit";
import { message, setError, type FormPathLeavesWithErrors, type SuperValidated } from "sveltekit-superforms";
...
export class SaveError<TOut extends Record<string, unknown>, TIn extends Record<string, unknown> = TOut> {
	...
	constructor(
		public error: string,
		protected options?: Partial<{ // [!code focus]
			// [!code word:ExtractBrand\<FormPathLeavesWithErrors\<TOut\>\>]
			field: ExtractBrand<FormPathLeavesWithErrors<TOut>>; // [!code focus] // [!code ++]
			status: NumericRange<400, 599>; // [!code focus]
		}> // [!code focus]
	) {
		if (options?.status) this.status = options.status;
	}
	...
	toForm(form: SuperValidated<TOut, App.Superforms.Message, TIn>) {
		return this.options?.field
			// [!code word:\ as\ FormPathLeavesWithErrors\<TOut\>]
			? setError(form, this.options.field as FormPathLeavesWithErrors<TOut>, this.error, { // [!code ++] // [!code focus]
					status: this.status // [!code focus]
				}) // [!code focus]
			: message(
					form,
					{
...
<Fragment slot="tab-2">$lib/brand.ts</Fragment>
<Fragment slot="panel-2">
```ts
declare const __brand: unique symbol;
